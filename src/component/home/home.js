...
function HandleOpenCloseNewBoardDialog(boards, setBoard) {
  ...
  const HandleAddBoard = (boardName) => {
    ...
    axios
      .post(`${config.path}/board/my-board`, { new_board: new_board })
      .then((result) => {
        console.log(result);
        const temp = {
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    axios.get(`${config.path}/board/my-board`).then((result) => {
  ...
  const HandleDeleteBoard = (boardID) => {
    axios
      .post(`${config.path}/board/delete-board?board_id=${boardID}`, {
        boardID: boardID,
      })
      .then((result) => {
        const curBoards = [];
        boards.forEach((element) => {
          if (element.boardID !== boardID) curBoards.push(element);
        });
        if (result.data.code === 0) {
          setBoard(curBoards);
        }
  ...
  const HandleEditBoard = (event, boardName) => {
    ...
    axios
      .post(`${config.path}/board/edit-board?board_id=${curBoard.boardID}`, {
      ...
      .then((result) => {
        ...
        const curBoards = [];
        boards.forEach((element) => {
          if (element.boardID === curBoard.boardID) curBoards.push(updated);
        ...
        <Grid container spacing={5} alignItems="flex-end">
          {boards.map((board) => (
            ...
            <Grid
              ...
              key={board.boardID}
              xs={12}
              sm={board.title === "Enterprise" ? 12 : 6}
              md={4}
            >
              <Card>
                <CardHeader
                ...
                <CardActions>
                  <Link
                    to={`/board?board_id=${board.boardID}`}
                    fullWidth
                    component={Button}
                  >
                    View
