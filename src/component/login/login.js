...
export default function Login() {
    ...
    const handleLogin = (username, password, event) => {
        event.preventDefault();
        axios.post(`${config.path}/user/login`, { username: username, password: password })
            .then(result => {
            ...
            <Container maxWidth="sm" component="main" className={classes.root}>
                <form onSubmit={(event) => handleLogin(values.username, values.password, event)}>
                    ...
                    <Typography align="center" gutterBottom>
                        <Link component={Button} type="submit" disabled={!valadateForm()} className={classes.link}>Login</Link>
                    </Typography>
                    <Typography align="center" gutterBottom>
                        <Link to={`/register`} variant="subtitle2" color="textPrimary" gutterBottom >
                            Or Register
                    </Link>
                    </Typography>
                </form>
